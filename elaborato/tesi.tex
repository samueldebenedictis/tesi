...
% Pannello di uno storyboard: schermata con didascalia numerata
...
La preparazione è comune alle due modalità ed è il momento in cui l'educatore adatta il gioco al gruppo (figura~\ref{fig:sb-prep}, tabella~\ref{tab:sb-prep}).

\begin{figure}[htbp]
    \centering
    \storypanel{mode-selection}{Passo 1: scelta della modalità.}{0.32\textwidth}\hfill
    \storypanel{home-single-filled}{Passi 2--5 e 7: configurazione (schermo singolo).}{0.32\textwidth}\hfill
    \storypanel{home-multi-config}{Passi 3--5 e 7: configurazione (multi-dispositivo).}{0.32\textwidth}
    \caption{Preparazione della partita.}
    \label{fig:sb-prep}
\end{figure}
...
Tutti i partecipanti guardano lo stesso schermo. L'educatore conduce la partita e resta il riferimento per la valutazione delle sfide (figura~\ref{fig:sb-single}, tabella~\ref{tab:sb-single}).

\begin{figure}[htbp]
    \centering
    \storypanel{dice-roll}{Passo 1: lancio del dado.}{0.48\textwidth}\hfill
    \storypanel{quiz-modal-answer}{Passi 3--4: sfida (Quiz) e registrazione dell'esito.}{0.48\textwidth}

    \vspace{0.8em}

    \storypanel{skip-turn-modal}{Passo 4: turno saltato.}{0.48\textwidth}\hfill
    \storypanel{end-game}{Passo 6: fine della partita.}{0.48\textwidth}
    \caption{Sessione a schermo singolo.}
    \label{fig:sb-single}
\end{figure}
...
3 & Se la casella è speciale, conduce la sfida mostrata nella finestra modale (figure~\ref{fig:modali-1} e~\ref{fig:modali-2}). & Estrae la carta dal mazzo corrispondente e la mostra a tutti. & Esercitare la competenza associata alla sfida (tabella~\ref{tab:sfide}). \\ \hline
...
Ogni giocatore usa il proprio smartphone, mentre il tabellone resta sullo schermo dell'educatore (figura~\ref{fig:sb-multi}, tabella~\ref{tab:sb-multi}). Le scelte tecniche che stanno dietro a questi passi sono descritte nella sezione~\ref{sec:multi-dispositivo}.

\begin{figure}[htbp]
    \centering
    \storypanel{multiplayer-lobby}{Passo 1: lobby con codice QR.}{0.5\textwidth}\hfill
    \storypanel{multiplayer-join}{Passo 2: registrazione dal telefono.}{0.2\textwidth}\hfill
    \storypanel{player-active-turn}{Passo 4: giocatore di turno.}{0.2\textwidth}

    \vspace{0.8em}

    \storypanel{multiplayer-board}{Passi 3--5: tabellone dell'educatore.}{0.5\textwidth}\hfill
    \storypanel{player-waiting-turn}{Passo 4: gli altri giocatori.}{0.2\textwidth}\hfill
    \storypanel{player-game-over}{Passo 6: fine partita.}{0.2\textwidth}
    \caption{Sessione multi-dispositivo.}
    \label{fig:sb-multi}
\end{figure}
...
Questa sezione descrive le singole schermate incontrate nello storyboard. La navigazione avviene tramite un header persistente che espone due elementi: un link diretto alla schermata delle istruzioni e un pulsante Menu che apre una finestra modale con i collegamenti a Nuova partita, Continua partita, Carica partita e Feedback.
...
Indovina l'emozione, Emozioni in musica, Test fisico, Cosa faresti se\ldots & Condivisa: visibile a tutti. & L'educatore registra l'esito al termine. \\ \hline
Battaglia & I due giocatori coinvolti sfidano dai rispettivi dispositivi. & Il server determina l'esito e aggiorna le posizioni. \\ \hline
...
Le schermate specifiche di questa modalità --- lobby con codice QR, pagina di registrazione e schermata personale del giocatore --- compaiono nello storyboard (figura~\ref{fig:sb-multi}).
