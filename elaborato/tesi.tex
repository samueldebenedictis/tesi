...
\begin{figure}[htbp]
    ...
    \end{tabular}
    \caption{Finestre modali delle sfide (1/4): Mimo, Quiz.}
    \label{fig:modali-1}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \end{tabular}
    \caption{Finestre modali delle sfide (2/4): Parole alle spalle, Disegno dettato.}
    \label{fig:modali-1b}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \end{tabular}
    \caption{Finestre modali delle sfide (3/4): Emozioni in musica, Indovina l'emozione.}
    \label{fig:modali-2}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \end{tabular}
    \caption{Finestre modali delle sfide (4/4): Cosa faresti se\ldots, Test fisico.}
    \label{fig:modali-2b}
\end{figure}
...
\begin{itemize}
    \item sulla pagina host (\texttt{app/multiplayer/[sessionId]/page.tsx}), \texttt{game-store} viene reidratato in sola lettura a ogni aggiornamento del \textit{polling} (\texttt{gameStore.setGame(Game.fromJSON(session.gameState))}) e serve esclusivamente a renderizzare il tabellone; le azioni dell'educatore (avvio partita, risoluzione di una sfida, designazione di un vincitore) non passano per le azioni dello store ma vengono inviate come richieste \texttt{POST} dirette agli endpoint \texttt{/api/sessions/[id]/*};
    \item le pagine \texttt{app/player/[sessionId]/[playerId]/page.tsx} e \texttt{app/join/[sessionId]/page.tsx} non importano né \texttt{useGameStore} né \texttt{useConfigStore}: si basano unicamente su \texttt{useState} locale e sull'hook \texttt{useSessionPolling} (\texttt{src/lib/use-session-polling.ts}), che espone direttamente lo stato pubblico di sessione ricevuto dal server.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Animazioni di transizione facciale} --- per il mazzo ``Indovina l'emozione'' è stata sperimentata una pipeline automatica di \textit{face morphing}: a partire dalla coppia di fotografie neutro/espressione dello stesso soggetto del database FACES, un rilevatore di punti di riferimento facciali (\textit{landmark}) individua i tratti del volto, che vengono poi triangolati e deformati progressivamente (\textit{warping}) con dissolvenza incrociata tra i due scatti, generando un'animazione fluida della transizione dall'espressione neutra a quella emotiva. L'idea è rendere più evidente ai giocatori la dinamica dei tratti distintivi di ciascuna emozione, invece della sola foto statica. La pipeline è funzionante ma non ancora integrata nell'interfaccia di gioco né validata con l'utenza.
\end{itemize}
