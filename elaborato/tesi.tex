...
Il panorama descritto evidenzia una convergenza verso strumenti digitali motivanti e strutturati, capaci di favorire la generalizzazione delle competenze apprese. La ricerca si è tuttavia concentrata quasi esclusivamente su software e ambienti virtuali sviluppati ex novo, trascurando un formato che coniuga struttura, interazione faccia a faccia e flessibilità configurativa: il gioco da tavolo in versione digitale.

\textit{La Città degli Imprevisti} si inserisce in questo spazio. A differenza dei serious games recensiti da Carneiro et al. \cite{carneiro2024} --- progettati per un uso individuale o mediato dallo schermo --- il gioco mantiene la struttura dell'interazione attorno a un tavolo condiviso, richiedendo la presenza fisica dei partecipanti e la mediazione di un educatore. I domini che esercita --- riconoscimento delle emozioni, comunicazione non verbale, prospettiva altrui, collaborazione, gestione della competizione --- corrispondono esattamente a quelli che la letteratura identifica come aree di intervento prioritarie nel DSA.
...
Il middleware \texttt{persist} consente di sincronizzare automaticamente gli store con il \texttt{localStorage} del browser, rendendo lo stato della partita sopravvivente al ricaricamento della pagina senza codice aggiuntivo.
