...
\textit{La Città degli Imprevisti} nasce da una collaborazione diretta tra utenti e docenti del CFPIL di Varese, che hanno progettato insieme un gioco centrato sulle competenze sociali.
La struttura di base è quella del Gioco dell'Oca --- un percorso lineare, un dado, le pedine --- arricchita con caselle speciali che attivano minigiochi a carattere sociale ed emotivo.
...
\begin{itemize}
    ...
    \item \textbf{Battaglia} --- Quando due giocatori si trovano sulla stessa casella devono sfidarsi a morra cinese (carta-forbice-sasso); introduce la competizione regolamentata e la gestione dell'esito (vittoria e sconfitta).
...
\section{Struttura ad alto livello}
...
Il modulo di \textbf{configurazione} permette di definire i parametri della partita e genera il tabellone al termine.

Il modulo di \textbf{gioco} gestisce lo svolgimento turno per turno fino a quando un giocatore raggiunge l'ultima casella.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{immagini/screenshots/home-filled}
    \caption{Modulo di configurazione: impostazione del numero di giocatori, lunghezza del percorso e tipologie di caselle.}
    \label{fig:home}
\end{figure}
...
\begin{itemize}
    ...
    \item scegliere quali tipologie di caselle speciali includere tra: Mimo, Quiz, Parole alle spalle, Disegno dettato, Emozioni in musica, Indovina l'emozione, Cosa faresti se\ldots, Test fisico, Caselle movimento;
    \item regolare la percentuale di caselle speciali rispetto al totale.
\end{itemize}

La tipologia \textbf{Battaglia} non è selezionabile: si attiva automaticamente in caso di collisione tra pedine. Per le \textbf{Caselle movimento}, la modalità avanzata consente di specificare il valore esatto di avanzamento o retrocessione per ogni singola casella.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{immagini/screenshots/dice-roll}
    \caption{Animazione del lancio del dado durante il turno del giocatore attivo.}
    \label{fig:dice-roll}
\end{figure}

Le sfide attivabili sono le dieci descritte nella sezione~\ref{sec:lcdi}: Mimo, Quiz, Parole alle spalle, Disegno dettato, Emozioni in musica, Indovina l'emozione, Cosa faresti se\ldots, Test fisico, Battaglia e Caselle movimento. Ciascuna viene presentata attraverso una finestra modale sovrapposta al tabellone al momento dell'attivazione, come mostrato nelle figure~\ref{fig:modali-1} e~\ref{fig:modali-2}.

\begin{figure}[htbp]
    \centering
    \begin{minipage}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{immagini/screenshots/mime-modal-topic}
        \par\smallskip\small Mimo.
    \end{minipage}
    \hfill
    \begin{minipage}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{immagini/screenshots/quiz-modal-answer}
        \par\smallskip\small Quiz.
    \end{minipage}
    ...
    \begin{minipage}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{immagini/screenshots/backwrite-modal-word}
        \par\smallskip\small Parole alle spalle.
    \end{minipage}
    \hfill
    \begin{minipage}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{immagini/screenshots/dictation-draw-modal-image}
        \par\smallskip\small Disegno dettato.
    \end{minipage}
    \caption{Finestre modali delle sfide (1/2): Mimo, Quiz, Parole alle spalle, Disegno dettato.}
    \label{fig:modali-1}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \begin{minipage}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{immagini/screenshots/music-emotion-modal}
        \par\smallskip\small Emozioni in musica.
    \end{minipage}
    \hfill
    \begin{minipage}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{immagini/screenshots/face-emotion-modal-answer}
        \par\smallskip\small Indovina l'emozione.
    \end{minipage}
    ...
    \begin{minipage}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{immagini/screenshots/what-would-you-do-modal}
        \par\smallskip\small Cosa faresti se\ldots
    \end{minipage}
    \hfill
    \begin{minipage}[b]{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{immagini/screenshots/physical-test-modal}
        \par\smallskip\small Test fisico.
    \end{minipage}
    \caption{Finestre modali delle sfide (2/2): Emozioni in musica, Indovina l'emozione, Cosa faresti se\ldots, Test fisico.}
    \label{fig:modali-2}
\end{figure}
...
Contiene un modulo strutturato per raccogliere valutazioni sull'applicazione da parte di docenti e alunni del CFPIL.
Il modulo richiede il nome del compilatore, il ruolo (docente, alunno o altro) e il tipo di esperienza con il gioco (versione fisica, digitale o entrambe).
Segue il questionario standardizzato \textit{System Usability Scale} (SUS)~\cite{brooke1996sus}, composto dalle 10 affermazioni originali tradotte in italiano, ciascuna valutata su scala 1--5.
Una sezione di valutazione numerica (scala 1--5) raccoglie infine giudizi su fedeltà della versione digitale rispetto a quella fisica (facoltativa), chiarezza dei meccanismi di gioco, grafica, gradimento complessivo e divertimento.
Tre campi di testo libero permettono di descrivere gli aspetti positivi, gli aspetti negativi e i suggerimenti per miglioramenti futuri.
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{immagini/screenshots/end-game}
    \caption{Schermata di fine partita con classifica finale dei giocatori.}
    \label{fig:end-game}
\end{figure}
...
La modalità \textbf{multi-dispositivo} è accessibile dalla schermata iniziale scegliendo la voce \textbf{Multi-dispositivo} al posto di \textbf{Schermo singolo}. L'educatore configura la partita e crea la sessione; sullo schermo compare la \textit{lobby} con un codice QR (figura~\ref{fig:multiplayer-lobby}).

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{immagini/screenshots/multiplayer-lobby}
    \caption{Schermata di \textit{lobby}: giocatori connessi e codice QR per l'accesso alla sessione.}
    \label{fig:multiplayer-lobby}
\end{figure}

Ogni giocatore scansiona il codice con il proprio smartphone, accede alla pagina di registrazione (figura~\ref{fig:multiplayer-join}) e inserisce il proprio nome. Una volta presenti tutti i partecipanti, l'educatore avvia la partita.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.4\textwidth]{immagini/screenshots/multiplayer-join}
    \caption{Pagina di accesso tramite QR code: il giocatore inserisce il proprio nome per unirsi alla sessione.}
    \label{fig:multiplayer-join}
\end{figure}

Ogni giocatore dispone di una schermata personale sul proprio dispositivo: quando è il suo turno, un pulsante permette di lanciare il dado; negli altri turni la schermata mostra il nome del giocatore attivo. Le sfide vengono distribuite tra i dispositivi preservando, ove pertinente, l'elemento di segretezza (figura~\ref{fig:player-screens}).

\begin{figure}[htbp]
    \centering
    \begin{minipage}[b]{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{immagini/screenshots/player-active-turn}
        \par\smallskip\small Turno attivo.
    \end{minipage}
    \hfill
    \begin{minipage}[b]{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{immagini/screenshots/player-waiting-turn}
        \par\smallskip\small Turno degli altri.
    \end{minipage}
    \caption{Schermata del giocatore nella modalità multi-dispositivo.}
    \label{fig:player-screens}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \begin{minipage}[b]{0.55\textwidth}
        \centering
        \includegraphics[width=\textwidth]{immagini/screenshots/multiplayer-quiz-host}
        \par\smallskip\small Tabellone dell'educatore: domanda visibile, con pulsante per rivelare la risposta corretta.
    \end{minipage}
    \hfill
    \begin{minipage}[b]{0.35\textwidth}
        \centering
        \includegraphics[width=\textwidth]{immagini/screenshots/multiplayer-quiz-player}
        \par\smallskip\small Smartphone del giocatore: stessa domanda, in attesa del giudizio dell'educatore.
    \end{minipage}
...
\begin{figure}[htbp]
    ...
    \begin{minipage}[b]{0.55\textwidth}
        \centering
        \includegraphics[width=\textwidth]{immagini/screenshots/multiplayer-physical-test-host}
        \par\smallskip\small Tabellone dell'educatore.
    \end{minipage}
    \hfill
    \begin{minipage}[b]{0.35\textwidth}
        \centering
        \includegraphics[width=\textwidth]{immagini/screenshots/multiplayer-physical-test-player}
        \par\smallskip\small Smartphone del giocatore.
    \end{minipage}
...
\begin{figure}[htbp]
    ...
    \begin{minipage}[b]{0.55\textwidth}
        \centering
        \includegraphics[width=\textwidth]{immagini/screenshots/multiplayer-dictation-draw-host}
        \par\smallskip\small Tabellone dell'educatore: nessun dettaglio della sfida è visibile, solo i nomi dei giocatori coinvolti.
    \end{minipage}
    \hfill
    \begin{minipage}[b]{0.35\textwidth}
        \centering
        \includegraphics[width=\textwidth]{immagini/screenshots/multiplayer-dictation-draw-player}
        \par\smallskip\small Smartphone del giocatore che descrive: tema e immagine di riferimento.
    \end{minipage}
...
\begin{figure}[htbp]
    ...
    \begin{minipage}[b]{0.55\textwidth}
        \centering
        \includegraphics[width=\textwidth]{immagini/screenshots/multiplayer-mime-host}
        \par\smallskip\small Tabellone dell'educatore: il mimo da eseguire è nascosto, pronto per essere rivelato su richiesta.
    \end{minipage}
    \hfill
    \begin{minipage}[b]{0.35\textwidth}
        \centering
        \includegraphics[width=\textwidth]{immagini/screenshots/multiplayer-mime-player}
        \par\smallskip\small Smartphone del giocatore che mima: la parola o frase da mimare.
    \end{minipage}
...
\begin{itemize}
    ...
    \item \texttt{app/multiplayer/[sessionId]/page.tsx} --- tabellone host per la sessione multi-dispositivo;
    \item \texttt{app/player/[sessionId]/[playerId]/page.tsx} --- schermata del giocatore sullo smartphone;
    \item \texttt{app/join/[sessionId]/page.tsx} --- pagina di accesso tramite QR code, reindirizza alla registrazione;
...
\begin{enumerate}
    \item \textbf{Vercel KV} --- in produzione, lo stato è memorizzato su Vercel KV, un servizio di \textit{key-value store} basato su Redis ospitato sulla piattaforma Vercel;
...
\begin{enumerate}
    ...
    \item Quando è il proprio turno, il giocatore attivo lancia il dado dal proprio dispositivo; il risultato viene comunicato al server e propagato agli altri partecipanti.
    ...
    \item Se la casella attiva una sfida, la gestione varia in base alla tipologia:
    \begin{itemize}
        \item \textbf{Quiz} --- il testo della domanda è visibile a tutti; l'educatore mostra la risposta corretta premendo il pulsante \textbf{Mostra risposta} e registra l'esito (successo o insuccesso);
        \item \textbf{Mimo e Disegno dettato} --- il tema da rappresentare compare esclusivamente sul dispositivo del giocatore attivo; gli altri vedono solo il tipo di sfida. Al termine, l'educatore seleziona quale giocatore ha indovinato, e quest'ultimo riceve il beneficio;
        \item \textbf{Parole alle spalle} --- la parola da tracciare compare sul dispositivo del giocatore attivo; il vincitore è automaticamente il compagno designato come destinatario della comunicazione, senza necessità di selezione aggiuntiva;
        \item \textbf{Emozione facciale, Emozione musicale, Test fisico, Cosa faresti se\ldots} --- la sfida è visibile a tutti; l'educatore registra l'esito al termine;
        \item \textbf{Battaglia} --- i due giocatori coinvolti effettuano la sfida sui rispettivi dispositivi; il server determina l'esito e aggiorna le posizioni.
    \end{itemize}
